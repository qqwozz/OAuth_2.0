...
func (s *server) logoutHandler(ctx *gin.Context) {
	// delete all the cookies and sessions values
	...
	// call auth0 logout endpoint to clear session and token from auth0 side
	logoutURL, err := url.Parse("https://"+os.Getenv("AUTH-DOMAIN")+"/v2/logout")
	if err != nil {
	...
	parametrs.Add("returnTo", redirectionUrl.String())
	parametrs.Add("client_id", os.Getenv("AUTH0_CLIENT_ID"))
	logoutURL.RawQuery = parametrs.Encode()
...
func (s *server) callbackHandler(ctx *gin.Context) {
    ...
    // TODO: Проверить, что state не пустой
    session := sessions.Default(ctx)
	...
	//get user infomation to display in profile
	...
	// ( Note: cookie should be encrypted before st )
	// u -> userInfo
    ...
    // TODO: Рассмотреть возможность сохранения только нужных полей
...
func main() {
    ...
    // TODO: Вынести в отдельную функцию инициализации
    ...
    // TODO: Проверить, что секрет соответствует требованиям безопасности
    for len(secret) < 32 {
        secret += "0"
    }
    ...
    // TODO: Добавить middleware для проверки аутентификации
    ...
    // Выход из системы
    server.router.GET("/logout", func(ctx *gin.Context) {
        session := sessions.Default(ctx)
        session.Clear()
        session.Save()
        ctx.Redirect(http.StatusTemporaryRedirect, "/")
...
// Implement authenticaton middleware to make sure user is auth before taking to
//protected endpoint
func IsAuthenticate() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		accessToken, err := ctx.Cookie("at")
		if err != nil {
			//Cookie does not exist, hence abort and redirect user to home or login page
			ctx.Redirect(http.StatusTemporaryRedirect, "/")
			ctx.Abort()
			return 
		}
		...
		// if evthng is okey, frwd the request to the handler
		ctx.Next()
	}
}
